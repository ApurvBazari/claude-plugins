...
| 10 | Every sheet-kind id in `SURF` has a `<dialog id="sheet-<id>">` | For each `SURF[id]==='sheet'`, the pre-rendered `<dialog class="sheet" id="sheet-<id>">` exists in `{{SHEETS}}` -- otherwise `openSheet` opens nothing. |
