...
### Added
- **Provenance footer in every generated document.** A walkthrough is a self-contained HTML file meant to be shared, but it carried no mark of what produced it. Generated documents now end with a quiet mono footer — `generated by walkthrough`, linking to the marketplace repo. It is emitted verbatim by all four render paths (`page-scaffold.md` is the single source they each copy), styled from existing theme tokens so light and dark are both covered, and placed outside `<main>` so the IntersectionObserver reveal can never hide it. It carries no tracking parameters and no session data.
- `document`'s chrome-rebrand rule and `update`'s reconstruction mapping now both name the footer explicitly — the rebrand retains it as provenance rather than stripping it as session branding, and reconstruction recovers it into no model field.
- **The detail-pane close control is keyboard-operable.** It was a `span` with an `onclick` and no role, tabindex or label, which failed the plugin's own self-check row 23 in every document it produced; it now carries `role="button"`, `tabindex="0"` and an `aria-label`.
