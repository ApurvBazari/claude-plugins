...
(function () {
  ...
  function initGraph() {
    ...
    var cy = window.cytoscape({
      ...
      style: [
        ...
        { selector: 'node[type="adr"]', style: {
          'background-color': '#1a1714', 'color': '#faf8f3',
          'border-color': '#1a1714', 'font-weight': 600, 'font-size': 12,
        }},
        { selector: 'node[type="opt-chosen"]', style: {
          'background-color': '#e8efe1', 'border-color': '#4a6b3a',
          'border-width': 2, 'color': '#4a6b3a', 'font-weight': 600,
        }},
        { selector: 'node[type="opt"]', style: {
          'background-color': '#f5f0e3', 'border-color': '#d4ccbc',
          'color': '#756f66', 'font-style': 'italic',
        }},
        { selector: 'node.highlight', style: {
          'border-color': '#b8651b', 'border-width': 2.5, 'background-color': '#faf2e3',
        }},
        { selector: 'edge', style: {
          'width': 1.2, 'line-color': '#d4ccbc',
          'target-arrow-color': '#d4ccbc', 'target-arrow-shape': 'triangle',
          'curve-style': 'bezier', 'arrow-scale': 0.9,
        }},
        { selector: 'edge.highlight', style: {
          'line-color': '#b8651b', 'target-arrow-color': '#b8651b', 'width': 2,
        }},
      ],
      layout: { name: window.cytoscapeDagre ? 'dagre' : 'breadthfirst', rankDir: 'LR', nodeSep: 28, rankSep: 70, edgeSep: 12 },
      minZoom: 0.5,
      maxZoom: 2.0,
    });

    // Hover: light up the touched node + its neighborhood.
    ...
    cy.on('mouseout', 'node', function () { cy.elements().removeClass('highlight'); });

    // Tap: scroll to the matching section AND open the side panel for it.
    cy.on('tap', 'node', function (e) {
      var id = e.target.id();
      var sectionEl = document.getElementById(id);
      if (sectionEl) {
        sectionEl.scrollIntoView({ behavior: 'smooth', block: 'start' });
        // If the section has a header that carries section metadata, open it.
