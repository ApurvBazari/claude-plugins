...
Invoke `lens:engine` (Skill tool), passing the target + the project finders registered in settings +
`taskIds = { scope, intent, analyze, verify }` so the engine transitions those four stages as it runs. It
returns a `review-findings` object in context. The full engine input/return contract is declared in
`../engine/references/engine-api.md`.
...
- **In-session task list (standalone only).** `/lens:review` surfaces progress via a `TaskCreate` list per `references/task-tracking.md`. Subjects are display-only; finder subagents are task-blind.
