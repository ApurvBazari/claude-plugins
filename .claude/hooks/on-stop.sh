...
MODIFIED_PLUGINS="$(git diff --name-only HEAD 2>/dev/null | grep -E '^(onboard|notify|devkit)/' | head -5)"
UNTRACKED_PLUGINS="$(git ls-files --others --exclude-standard 2>/dev/null | grep -E '^(onboard|notify|devkit)/' | head -5)"
