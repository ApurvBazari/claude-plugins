...
Structure is only half the picture. After `/validate` passes, run **`/doc-audit`** to verify the
human docs match what's shipped (per-plugin Skills sections, root command index, manifest/desc
sync, docs-site parity).
