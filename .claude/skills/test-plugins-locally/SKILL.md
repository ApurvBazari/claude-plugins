...
The four plugins in this repo (`onboard`, `greenfield`, `notify`, `handoff`) ship through the `apurvbazari-plugins` local marketplace which points at this directory. Plugins are **copied** into `~/.claude/plugins/cache/apurvbazari-plugins/<plugin>/<version>/` on install, so source edits don't reach the running session until the cache is refreshed.
...
- **Setup** — first-time wiring: register marketplace if missing, install all 4 plugins, create the sibling testbed dir
...
for p in onboard greenfield notify handoff; do
...
Surface anything noteworthy: stale cache, orphan plugin entries (e.g. `forge@apurvbazari-plugins` left over from the forge→greenfield rename — suggest `claude plugin uninstall forge@apurvbazari-plugins` if found), uncommitted source changes that won't be visible until reset.
...
# 2. Install all 4 plugins (skip already-installed)
for p in onboard greenfield notify handoff; do
...
```bash
TESTBED="$(cd .. && pwd)/claude-plugins-testbed"
SANDBOX="$TESTBED/greenfield-test-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$SANDBOX"
echo "cd $SANDBOX && claude"
echo "then run: /greenfield:start"
```

Print the two-line instruction for the user — do NOT `cd` into the sandbox from this session (this Claude session is anchored to the source repo; running plugin commands here would test against the wrong working directory).

Smoke checklist after `/greenfield:start`:
- Visual companion opens in browser (or falls through with a one-line notice if Python 3 missing)
- Step 0 mini-wizard collects 6 answers
- Phase 1 phases unlock as `requires:` are approved
- `.claude/greenfield-state.json` exists in the sandbox after the first checkpoint
