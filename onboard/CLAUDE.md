...
- **Contract reference**: `skills/start/references/phase-tracking.md` is the authoritative design document for the tracking model, task-subject scheme (`onboard:phase:<N>:<slug>`), and resume semantics. The belt gate `.github/scripts/check-phase-tracking.sh` asserts the wiring is present in all four entry points.
