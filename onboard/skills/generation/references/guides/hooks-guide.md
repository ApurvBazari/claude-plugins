    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file=$(cat - | jq -r '.tool_input.file_path' 2>/dev/null || cat - | grep -o '\"file_path\": *\"[^\"]*\"' | head -1 | sed 's/.*: *\"//;s/\"//') && case \"$file\" in *.rs) rustfmt \"$file\" 2>/dev/null ;; esac; exit 0",
            "timeout": 10000
          }
        ]
      }
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file=$(cat - | jq -r '.tool_input.file_path' 2>/dev/null || cat - | grep -o '\"file_path\": *\"[^\"]*\"' | head -1 | sed 's/.*: *\"//;s/\"//') && case \"$file\" in *.ts|*.tsx|*.js|*.jsx) npx eslint --no-error-on-unmatched-pattern \"$file\" 2>&1 | head -20 ;; esac; exit 0",
            "timeout": 15000
          }
        ]
      }
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "file=$(cat - | jq -r '.tool_input.file_path' 2>/dev/null || cat - | grep -o '\"file_path\": *\"[^\"]*\"' | head -1 | sed 's/.*: *\"//;s/\"//') && case \"$file\" in *.py) ruff check \"$file\" 2>&1 | head -20 ;; esac; exit 0",
            "timeout": 10000
          }
        ]
      }
...
These templates implement the Plugin Integration enforcement philosophy: boundary enforcement (blocking) at commit, advisory reminders at session-start and feature-start. Generated by onboard when `callerExtras.qualityGates` is present in programmatic context.
...
```bash
#!/usr/bin/env bash
set -u  # no -e / -o pipefail — see ../emission/hooks-generation.md § O7 § Shell options for hook scripts
...
> **Authoritative spec**: the full generation contract for this hook lives in [`../emission/hooks-generation.md` § O7 — Feature-start detector PreToolUse hook](../emission/hooks-generation.md). That section contains the **8 required behavioral invariants** (MUST-include checks) + the reference implementation. Do not maintain a duplicate template here — edit hooks-generation.md § O7 if you need to change detector behavior.

**Minimum checklist** (full invariant list in `../emission/hooks-generation.md` § O7):
...
**Generation rules** (condensed — full set in `../emission/hooks-generation.md` § O7):
...
- `#!/usr/bin/env bash` + `set -u` (never `set -euo pipefail` — see `../emission/hooks-generation.md` § Shell options for hook scripts)
...
**Inference trigger**: emit when agent-team mode is enabled. This replaces the prior inline template that lived in `../extended/evolution-hooks-guide.md`.
...
**Matcher**: not supported — omit.
**Inference trigger**: emit when agent-team mode is enabled. This replaces the prior inline template in `../extended/evolution-hooks-guide.md`.
...
# Advisory only by default. The test command below is substituted at generation time.

if ! __TEST_CMD__ >/dev/null 2>&1; then
...
Generation note: replace the `__TEST_CMD__` token with the literal test command discovered during analysis (`npm test`, `pytest`, `go test ./...`, `cargo test`, etc.) before writing the script. If no test command was detected, skip generating this hook entirely — do **not** emit an env-var fallback or advisory no-op. Never substitute with `eval` or a variable expansion: word-splitting of the literal command is sufficient and avoids shell-metacharacter interpretation.
...
**Inference trigger**: emit when `enriched.enableEvolution === true`. Existing drift-detection integration in `../extended/evolution-hooks-guide.md` consumes this; keep that guide authoritative for the drift scripts, this section only covers the generic advisory template.
...
- The legacy `FileChanged` drift pipeline (`detect-dep-changes.sh`, `detect-config-changes.sh`, `detect-structure-changes.sh`) stays in `../extended/evolution-hooks-guide.md`. That guide now cross-references this section for the base advisory template; the drift-specific scripts are unchanged.
- Team-mode `TaskCreated` / `TaskCompleted` behavior likewise lives in `../extended/agent-teams-guide.md` for team-composition concerns; this section owns the event-level generation contract.
...
2. **Validation** — apply the 10 rules in `../emission/hooks-generation.md` § Hook Type Validation before emitting any variant. Missing required fields drop the entry with the appropriate `skipped` reason.
