...
```bash
#!/usr/bin/env bash
set -u  # no -e / -o pipefail — see ../../SKILL.md § O7 § Shell options for hook scripts
...
- `#!/usr/bin/env bash` + `set -u` (never `set -euo pipefail` — see `../../SKILL.md` § Shell options for hook scripts)
