...
|---|---|
| `update` | 1. `.claude/onboard-meta.json.detectedPlugins.installedPlugins` (always populated by init/generate since v1.2.0, regardless of `pluginSource`) → 2. `.claude/onboard-meta.json.callerExtras.installedPlugins` (legacy pre-v1.2.0 projects) → 3. `.claude/greenfield-meta.json.generated.toolingFlags.installedPlugins` → 4. empty |
| `evolve` | 1. `.claude/greenfield-meta.json.generated.toolingFlags.installedPlugins` → 2. skip step entirely (evolve requires a greenfield baseline) |
...
{
  "driftReport": {
    "baselineSource": "onboard-meta | greenfield-meta | caller-extras | none",
...
- `onboard:evolve` → write `currentPlugins` to `.claude/greenfield-meta.json.generated.toolingFlags.installedPlugins` and refresh the other toolingFlags fields per `evolve/SKILL.md` Step 2b.3.
