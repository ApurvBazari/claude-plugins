...
**Merge semantics**: All quality-gate hooks merge into `.claude/settings.json` following the existing merge strategy (see `../guides/hooks-guide.md` § Settings Merge Strategy). If a hook with the same matcher/event already exists, skip don't duplicate.
...
See `../guides/hooks-guide.md` for generated script templates, ShellCheck requirements, and concrete examples of sessionStart + featureStart + preCommit hooks.
...
# Invariants: see § O7 in onboard's hooks-generation.md.
...
1. **Caller-provided**: `callerExtras.qualityGates.<event>[]` where `<event>` is one of `sessionEnd`, `userPromptSubmit`, `preCompact`, `subagentStart`, `taskCreated`, `taskCompleted`, `fileChanged`, `configChange`, `elicitation`. See `../../../../schemas/context-shape-v3.json` (`callerExtras.qualityGates`) for where this field sits in the context; the per-entry `hookType`/aux fields are specified in § Hook Type Validation below.
...
| Event | Inference trigger | Template script (in `../guides/hooks-guide.md`) |
|---|---|---|
...
| `FileChanged` | `enriched.enableEvolution === true` — use the drift-detection matcher set from `../extended/evolution-hooks-guide.md`; fall back to the generic lockfile matcher when the caller supplies no explicit matcher | `file-changed-notice.sh` or the drift scripts from evolution-hooks-guide |
...
|---|---|---|---|
| `command` | script basename (e.g., `session-end.sh`) | `${project}/.claude/hooks/<name>.sh` | template in `../guides/hooks-guide.md` |
...
1. **Matcher-incompatible events MUST NOT emit a `matcher` field** in the settings entry. Applies to: `SessionEnd`, `UserPromptSubmit`, `SubagentStart`, `TaskCompleted`. See `../guides/hooks-guide.md` § Matcher Compatibility for the authoritative table. Silently ignoring an extraneous matcher is not acceptable — the generated JSON must be honest.
...
3. **All advanced events are advisory by default** — the generated scripts always `exit 0`. The caller may upgrade `taskCreated` / `taskCompleted` to `mode: "blocking"` explicitly; all other events ignore `mode` (only advisory is supported because Claude Code does not honor `exit 2` on them). A `taskCreated` entry also accepts an optional `minSubjectLength` (number, default `10`): the generated `task-created-check.sh` raises its advisory only when the new task's subject is shorter than that many characters, so a project with intentionally terse task subjects can raise the threshold to quiet it.
4. **Script generation**: copy the corresponding template from `../guides/hooks-guide.md` § Advanced Event Templates into `<project>/.claude/hooks/<script-name>`, make executable (`chmod +x`), verify `shellcheck -x` passes. Do NOT re-author the templates inline — the guide is authoritative.
...
**What to generate**: The script and settings.json entry from `../guides/hooks-guide.md` § WorktreeCreate hook (init.sh auto-runner).
