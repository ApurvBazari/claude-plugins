...
Headless callers (greenfield) read `mcpStatus` from the generate-skill return and can surface it in their own handoff output.
...
Used by greenfield when the scaffold template already ships an `.mcp.json`.
