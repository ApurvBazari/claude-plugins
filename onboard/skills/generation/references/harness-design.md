...
Onboard acts as the **Initializer Agent** — it runs once and sets up everything. The artifacts generated here enable every **subsequent Claude session** to quickly understand project state, pick up where the last session left off, and make incremental progress.
...
# Generated by onboard — run at the start of every Claude session
cd "$(dirname "$0")"
...
Feature list is generated during Phase 1 confirmation step. Onboard decomposes the app description into testable features, groups them into sprints, and shows the developer:
...
## Current State
- **Phase**: Initial setup complete (onboard scaffold)
- **Last session**: [date] — onboard initialization
...
### AI Feature Weaving
When decomposing features or adding new features, look for opportunities to weave AI-powered capabilities into the product:
...
└── (DISCUSSION.md, PLAN.md — user's    ├── feedback_*.md (workflow prefs)
     own docs, not onboard-generated)        └── reference_*.md (pointers)
...
Add to `onboard-meta.json` and `docs/progress.md`:
...
3. `docs/progress.md` — initialized with Session 1 (onboard start) entry
