...
Greenfield acts as the **Initializer Agent** — it runs once and sets up everything. The artifacts generated here enable every **subsequent Claude session** to quickly understand project state, pick up where the last session left off, and make incremental progress.
...
# Generated by Greenfield — run at the start of every Claude session
cd "$(dirname "$0")"
...
Feature list is generated during Phase 1 confirmation step. Greenfield decomposes the app description into testable features, groups them into sprints, and shows the developer:
...
## Current State
- **Phase**: Initial setup complete (Greenfield scaffold)
- **Last session**: [date] — Greenfield initialization
...
## Session Log
### Session 1 — [date] (Greenfield init)
...
### AI Feature Weaving
When decomposing features (Greenfield Phase 1) or adding new features, look for opportunities to weave AI-powered capabilities into the product:
...
└── (DISCUSSION.md, PLAN.md — user's    ├── feedback_*.md (workflow prefs)
     own docs, not Greenfield-generated)     └── reference_*.md (pointers)
...
Add to `greenfield-meta.json` and `docs/progress.md`:

```json
// greenfield-meta.json
{
  "costs": {
    "greenfieldInit": {
      "duration": "25 minutes",
...
3. `docs/progress.md` — initialized with Session 1 (Greenfield init) entry
