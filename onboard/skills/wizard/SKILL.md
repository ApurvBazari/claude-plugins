...
The `lspPlugins` field is the developer-accepted list of marketplace LSP plugins from Exchange 3 (LSP detection prompt). An empty array means "detected candidates but developer declined all"; an absent field means "programmatic path — full detected list is the implicit accept". `../generation/SKILL.md` § LSP Plugin Recommendations consumes this alongside `callerExtras.lspPlugins` and `callerExtras.disableLSP`. See `../generation/references/lsp-plugin-catalog.md` for the plugin→language mapping.
