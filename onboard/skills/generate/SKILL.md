...
```json
{
  ...
  "wizardAnswers": {
    ...
    "advancedHookEvents": "string[] (optional) — event names the developer explicitly selected in wizard Step 1. Empty array = suppress inference. Absent = inference runs. See ../generation/SKILL.md § Advanced Event Hooks § Wizard opt-in plumbing.",
    ...
    "lspPlugins": "string[] (optional) — developer-accepted list of marketplace LSP plugins from wizard Step 6. Empty array = 'detected but declined all'. Absent = internal generation (no wizard) — full detected list is implicit accept. See ../generation/SKILL.md § LSP Plugin Recommendations — emission Step 3 and ../generation/references/lsp-plugin-catalog.md.",
    "builtInSkills": "string[] (optional) — developer-accepted list of built-in Claude Code skill names from wizard Step 7. Empty array = 'candidates existed but declined all'. Absent = internal generation (no wizard) — full candidate list (core + fired extras) is implicit accept. See ../generation/SKILL.md § Built-in Claude Code Skills — emission Step 4 and ../generation/references/built-in-skills-catalog.md."
  },
