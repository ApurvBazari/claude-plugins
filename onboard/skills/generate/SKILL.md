...
```json
{
  ...
  "wizardAnswers": {
    ...
    "lspPlugins": "string[] (optional) — developer-accepted list of marketplace LSP plugins from wizard Step 6. Empty array = 'detected but declined all'. Absent = internal generation (no wizard) — full detected list is implicit accept. See ../generation/SKILL.md § LSP Plugin Recommendations — Phase 7c and ../generation/references/lsp-plugin-catalog.md.",
  ...
  "callerExtras": {
    ...
    "builtInSkills": "string[] (optional) — explicit list of built-in Claude Code skill names to document in the generated CLAUDE.md during Phase 7d. When present, skips wizard Step 7 and treats the array as the accepted list verbatim. Pass an empty array to record 'candidates existed but declined all'. When absent (and disableBuiltInSkills is not true), wizard Step 7 runs in interactive mode or defaults to the full candidate list (core + fired extras) during internal generation (no wizard). See ../generation/references/built-in-skills-catalog.md for valid skill names.",
    "qualityGates": {
