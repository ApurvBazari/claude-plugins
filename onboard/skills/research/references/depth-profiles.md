...
2. **Intersect** with the detected source roots from the engine's SCOPE step (Step 2) so a glob never escapes the real source tree — except **root-dwelling globs** (manifests/lockfiles, docs, test/lint config) which are exempt from the intersection (see SKILL § Step 2), since they legitimately live at the repo root (the detected-roots set may be supplied by the caller via `reconHints.detectedRoots`; otherwise the engine self-detects).
