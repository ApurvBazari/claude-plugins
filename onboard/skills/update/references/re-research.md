...
4. **Generate** — invoke `Skill(onboard:generate)` with that context. `generate` threads `research` + the `reResearch` marker to `config-generator`, which re-sharpens all artifacts honoring the customization floor + marker surgery (`../../generation/references/research/re-research-merge.md`) and merges the verify backlog (`../../generation/references/research/verify-backlog-seeding.md`).
