...
Locate the per-phase template at `${CLAUDE_PLUGIN_ROOT}/skills/synthesis-review/references/templates/<phaseId-lowercase>-<short-name>.html`. For Round 1: `p8-cicd.html`.
...
2. Look up `context.dependencies[phaseId]` — the list of paths this phase depends on (e.g., `["P0.willDeploy", "P3.databaseHost"]`).
3. For each dependency, resolve the current value from `context.phases.<sourcePhase>.<remaining-path>` and inject a cross-check note: "Assumes [dependency-path] = [value]." If the dependency was not yet captured at synthesis time, annotate "not yet captured" — do not block.
...
Refer to `references/section-prompts.md` for the per-section composition strategy and the Round 1 P8 section map.
...
Write to `<targetProjectRoot>/docs/architecture/<phaseId-lowercase>-<short-name>-dependencies.json` via atomic `.tmp` + rename.

## Step 4: Walk the developer through the synthesis

Tell the developer the HTML is ready:

> I've written the **Phase {phaseId}** synthesis to `docs/architecture/{file}.html`. Opening it in your browser will help:
>
...
2. Substitute the architectural-file patterns based on `context.phases.P2.stack` (e.g., `src/**/*.ts` for Next.js, `prisma/**/*.prisma` for Prisma schemas). Patterns go into the `{{ARCH_PATTERNS}}` placeholder as a bash-array body.
...
Write `context.syntheses[phaseId]` to `greenfield-state.json` via the standard atomic `.tmp` + rename pattern (see `init/SKILL.md` § State persistence).
