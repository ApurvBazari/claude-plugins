...
The categories are evaluated in order. For each, check the "Skip if" condition first; only ask the question if the condition is `false`. Apply each answer back to `greenfield-state.json.context` before moving to the next category.
...
2. If selected, append an entry to `greenfield-state.json.parkedQuestions[]` with `{ category, question, placeholder, why }`.
...
- Multiple real greenfield runs have surfaced the same class of contradiction in a sprint review.
- The contradiction is detectable from `greenfield-state.json.context` alone (no extra tool calls needed).
...
- **Code-level architecture decisions** (folder structure, design patterns) — that's the Generator's job in implementation sessions, per the Planner Scope Principle in `greenfield/skills/context-gathering/SKILL.md`.
- **Tech-stack version pinning** — that's `greenfield/agents/stack-researcher.md`'s job during Phase 1 Step 2.
