---
name: scaffolding
description: Greenfield Phase 2 — executes the project scaffold (CLI, from-scratch, template, or walking-skeleton), sets up git, runs hello-world verification. Internal building block invoked by greenfield init — not user-invocable.
user-invocable: false
---

# Scaffolding Skill — Project Setup & Git Configuration

You are executing Phase 2 of Greenfield: scaffolding the application and setting up git infrastructure. All decisions were made during Phase 1 (context gathering). This phase is execution, not discussion.
...
4. If the developer anchors to a sibling:
   ...
   - Note the anchor in `greenfield-meta.json` for future reference
...
This path scaffolds ONLY one representative example of each architectural pattern — not a complete app. The goal is to give greenfield's AI-tooling phase (Phase 3) enough concrete patterns to derive project-specific CLAUDE.md, rules, and hooks from, without investing in full features yet.
...
**FORBIDDEN patterns** (every one observed in the 2026-04-16 greenfield run):
...
3. **`.gitignore`** — If not already present, create with standard patterns for the stack plus: `.env*` (except .env.example), `node_modules/`, build dirs, OS files, `.claude/settings.local.json`. **Also append greenfield + onboard runtime state files** (these persist wizard answers that may contain secrets and must never be committed):
   ```
...
Write `.claude/greenfield-meta.json`:
...
> 3. Abort the whole greenfield session
...
6. **Checkpoint after every Step** — Always write `greenfield-state.json` at Step boundaries so resume works.
