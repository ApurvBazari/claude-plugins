...
Phase 1: Context Gathering ──→ context-gathering skill (adaptive wizard)
     ...
     │                            ├── Step 19: Schema & API Draft Review (schemaDraftReview phase — Round 5 insert; 12 Qs heavy / 6 light + auto-render mid-flow)
     ...
     │                                                         Round 2 / 2.5 / 3 / 4 / 5: Step 2.2 → personas, Step 2.5 → architecturalFraming, Step 2.7 → domainModel, Step 3 → dataArchitecture, Step 4 → apiIntegration, Step 5 → auth, Step 6 → privacy, Step 7 → security, Step 8 → runtimeOperations, Step 11 → cicdAndDelivery, Step 15 → architecturalValidation, Step 16 → featureRoadmap, Step 19 → schemaDraftReview)
     │
...
- `context-gathering/SKILL.md` — Phase 1: adaptive state-machine wizard (3.0 Round 2 / 2.5 / 3 / 4 / 5 / 6: 30 wizard steps (1–15 + Round 4 inserts at 2.2 + 2.7 + Round 5 inserts at 16 + 19 + Round 6 inserts at 7, 9, 10, 13, 15 + Round 6 frontend trio at 22/23/24 + Round 6 i18n at 25 + Round 6 CI Draft at 20 + Round 6 plugin split at 21/30); Step 2.2 = Personas / personas (16 Qs heavy / 4 Qs light — Round 4), Step 2.5 = Architectural Framing / architecturalFraming (4 Qs — topology, deploymentShape, scaleTarget, boundaryNotes), Step 2.7 = Domain Modeling / domainModel (11 Qs Full DDD / ~8 DDD-lite / ~6 Light — Round 4), Step 3 = Data Architecture / dataArchitecture (12 Qs), Step 4 = API & Integration / apiIntegration (10 Qs), Step 5 = Auth & Identity / auth, Step 6 = Privacy & Data Governance / privacy, Step 7 = Search / search (11 Qs heavy / 6 light, flat — Round 6), Step 8 = Security / security, Step 9 = Caching / caching (12 Qs heavy / 7 light, flat — Round 6), Step 10 = Real-time / realtime (12 Qs heavy / 6 light, per-persona — Round 6), Step 11 = CI/CD & Delivery / cicdAndDelivery (17 Qs from Round 1), Step 13 = File Uploads & CDN / fileUploads (13 Qs heavy / 7 light, per-persona — Round 6), Step 15 = Payments / payments (14 Qs heavy / 7 light, per-persona — Round 6) — note Architectural Validation renumbered out of slot 15 in Round 6 to make room; final cross-phase sign-off now folds into the post-Round 6 wrap. Step 16 = Feature Roadmap / featureRoadmap (14 Qs heavy / 7 Qs light + per-persona auto-loop — Round 5), Step 19 = Schema & API Draft Review / schemaDraftReview (12 Qs / 6 light + auto-render hook — Round 5), Step 20 = CI Draft Review (auto-render via render-ci-drafts.sh dispatcher — Round 6), Step 21 = P7.5 Plugin Recommendation (recommendation mode — Round 6 split), Step 22 = P5 Frontend Architecture / frontendArchitecture (Round 6), Step 23 = P5.3 Design System / designSystem (Round 6), Step 24 = P5.6 UX / Accessibility / Performance / uxAccessibilityPerf (hosts 3 inline gates — Round 6), Step 25 = i18n / l10n / i18nL10n (Round 6), Step 30 = P10 Plugin Install (install mode — Round 6 split). Round 4 mode toggles (depth / coupling / domainFormat) set at Step 1.1 and gate downstream Q counts. Round 4 Q-bank flags: `showInLight` gates light-mode display; `isRiskCapture` routes answers to top-level `risks[]`; `loopOver`/`loopMode` drive per-persona/entity auto-loop. Total ~120 Qs heavy / ~65 Qs light depending on stack + deploy + persona/entity loop counts.)
- `synthesis-review/SKILL.md` — Phase 1.8: per-phase synthesis review. Renders `docs/adr/<topic-kebab>.html` in the scaffolded project, walks Approve/Adjust/Skip per section, writes `dependencies.json` sidecar, installs freshness hook. Invoked inline by `context-gathering` at the end of each major step that has a synthesis template (Round 2 / 2.5 / 3: architecturalFraming at Step 2.5, dataArchitecture at Step 3, apiIntegration at Step 4, auth at Step 5, privacy at Step 6, security at Step 7, runtimeOperations at Step 8, cicdAndDelivery at Step 11, architecturalValidation at Step 15). Round 4 templates added: `personas.html`/`md`, `domain-model.html`/`md`, `arch-val-risk-reconciliation-section.html` (fragment inserted into architectural-validation.html as Section 1). Round 5 templates added: `feature-roadmap.html`/`md`, `schema-draft-review.html`/`md`. Round 6 templates added: search.{html,md}, caching.{html,md}, realtime.{html,md}, file-uploads.{html,md}, payments.{html,md}, frontend-architecture.{html,md}, design-system.{html,md}, ux-accessibility-perf.{html,md}, i18n-l10n.{html,md}, ci-draft-review.{html,md}.
...
- Progress indicator: every wizard Step emits "Step X of 20" so sessions can't silently derail (20 = 15 R3 steps + Round 4 Step 2.2 Personas + Step 2.7 Domain Modeling + Round 5 Step 16 Feature Roadmap + Step 19 Schema & API Draft Review + the Phase 3 plugin steps + handoff)
...
- **Round 5 deterministic outputs:** `phases.featureRoadmap` drives mechanical generation of `docs/feature-list.json` + `docs/sprint-contracts/sprint-1.json`. `phases.schemaDraftReview.drafts.{db,api,event}.content` is written verbatim by onboard to canonical paths per `outputStrategy` (project-root or docs-drafts). Pre-R5 (alpha.5) sessions auto-migrate via `/greenfield:pickup` with `skipped: true` defaults — onboard falls back to interactive handoff in that case.
